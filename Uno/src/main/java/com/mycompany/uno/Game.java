...
public class Game {
	...
	public Game(String[] pids) {
		...
		for (int i= 0; i < pids.length; i++) {
			ArrayList<UnoCard> hand = new ArrayList<UnoCard>(Arrays.asList(deck.drawCard(4)));
			playerHand.add(hand);
		}
	}
	...
	public ImageIcon getTopCardImage() {
		return new ImageIcon(validColor + "_" + ".png");
	}
	...
	public void submitPlayerCard(String pid, UnoCard card, UnoCard.Color declaredColor)
		...
		if(hasEmptyHand(this.playerIds[currentPlayer])) {
			JLabel message2 = new JLabel(this.playerIds[currentPlayer] +"Selamat Telah Menang!!!");
			message2.setFont(new Font("Arial", Font.BOLD, 48));
			JOptionPane.showMessageDialog(null, message2);
			System.exit(0);
			}
		...
		if(card.getValue() == UnoCard.Value.DrawTwo){
			...
			getPlayerHand(pid).add(deck.drawCard());
			getPlayerHand(pid).add(deck.drawCard());
			JLabel message = new JLabel(pid +"Ambil 2 kartu");
		}
		...
		if(card.getValue() == UnoCard.Value.Wild_Four){
			...
			getPlayerHand(pid).add(deck.drawCard());
			getPlayerHand(pid).add(deck.drawCard());
			JLabel message = new JLabel(pid +"Ambil 4 kartu");
		}
		
		if (card.getValue() == UnoCard.Value.Skip) {
			JLabel message = new JLabel(playerIds[currentPlayer] + "Terlewati!");
			message.setFont(new Font("Arial", Font.BOLD, 48));
			JOptionPane.showMessageDialog(null, message);
			if (gameDirection == false) {
				currentPlayer = (currentPlayer + 1) % playerIds.length;
		...
		if(card.getValue() == UnoCard.Value.Reverse) {
			JLabel message = new JLabel(pid + "arah permainan terubah!");
			message.setFont(new Font("Arial", Font.BOLD, 48));
			JOptionPane.showMessageDialog(null, message);
